...
namespace CaptchaApp.Classes
{
    public class CaptchaGeneration
    {
        ...
        public TextBlock TBlock(TextBlock textBlock, char algorithm, Random random)
        {
        ...
        public void Generation(StackPanel panel) {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                {
                    case 0:
                        TBlock(textBlock, capitalAlphabet[index], rand);
                        panel.Children.Add(textBlock);
                        captcha += capitalAlphabet[index];
                        break;
                    case 1:
                        TBlock(textBlock, normalAlphabet[index2], rand);
                        panel.Children.Add(textBlock);
                        captcha += normalAlphabet[index2];
                        break;
                    case 2:
                      
                        TBlock(textBlock, numbers[index3], rand);
                        panel.Children.Add(textBlock);
                        captcha += numbers[index3];
                        break;
                    case 3:
                       
                        TBlock(textBlock, symbols[index4], rand);
                        panel.Children.Add(textBlock);
